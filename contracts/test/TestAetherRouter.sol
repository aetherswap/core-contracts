...
contract TestAetherRouter is IAetherSwapCallback {
    ...
    function aetherSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IAetherPool(msg.sender).token0() : IAetherPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IAetherPool(pools[0]).token1();
            IAetherPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IAetherPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IAetherPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
