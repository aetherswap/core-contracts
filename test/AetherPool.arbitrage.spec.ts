...
import { AetherPoolSwapTest } from '../typechain/AetherPoolSwapTest'
...
describe('AetherPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('AetherPoolSwapTest')
            const tester = (await testerFactory.deploy()) as AetherPoolSwapTest
