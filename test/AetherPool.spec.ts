...
import { TestAetherSwapPay } from '../typechain/TestAetherSwapPay'
...
import { TestAetherReentrantCallee } from '../typechain/TestAetherReentrantCallee'
...
describe('AetherPool', () => {
  ...
  let factory: AetherFactory
  let pool: MockTimeAetherPool

  let swapTarget: TestAetherCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeAetherPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestAetherReentrantCallee')
      ).deploy()) as TestAetherReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestAetherSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestAetherSwapPay')
      underpay = (await underpayFactory.deploy()) as TestAetherSwapPay
